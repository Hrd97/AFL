...
@bp_public.route('/public/homepage', methods=['GET', 'POST'])
def homepage():
    #Patient.query.filter(Patient.phoneNumber == phoneNumber).first()
    #session['user_id'] = 'H004'

    me =UserClass.getfromid(session["user_id"])

    project=me.publicproject()
    return render_template('publicproject/publicproject.html', projects=project)

@bp_public.route('/public/add', methods=['GET', 'POST'])
def add():
